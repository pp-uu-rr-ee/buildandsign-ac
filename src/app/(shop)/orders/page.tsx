  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6 dark:text-gray-400">
        ...
        <span className="text-gray-900 font-medium dark:text-gray-100">{t.orders.title}</span>
      </div>

      <h1 className="text-3xl font-bold text-gray-900 mb-8 dark:text-gray-50">{t.orders.title}</h1>
      ...
      {orders.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-300 p-16 text-center dark:border-gray-700">
          ...
          <p className="text-gray-500 font-medium mb-2 dark:text-gray-400">{t.orders.noOrders}</p>
          ...
          >
            {t.orders.shopAC} <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      ) : (
