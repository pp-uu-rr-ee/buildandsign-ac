...
export default async function AdminDashboardPage() {
  const [stats, recentOrders, { rows: upcomingBookings }, t, locale] = await Promise.all([
    getDashboardStats(),
    getRecentOrders(6),
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        <StatCard label={t.admin.dashStatsOrders} value={stats.totalOrders} sub={t.admin.dashStatsOrdersSub(stats.pendingOrders)} icon={ShoppingBag} color="blue" />
        <StatCard label={t.admin.dashStatsRevenue} value={formatPrice(stats.totalRevenue)} icon={ShoppingBag} color="green" />
        <StatCard label={t.admin.dashStatsBookings} value={stats.totalBookings} sub={t.admin.dashStatsBookingsSub(stats.pendingBookings)} icon={CalendarCheck} color="orange" />
        <StatCard label={t.admin.dashStatsCustomers} value={stats.totalCustomers} icon={Users} color="purple" />
      </div>
      ...
      {stats.lowStock > 0 && (
        <div className="flex items-center gap-3 rounded-xl bg-orange-50 border border-orange-200 px-5 py-3">
          ...
          <p className="text-sm text-orange-800">
            <span className="font-semibold">{t.admin.dashLowStock(stats.lowStock)}</span>{" "}
            <Link href="/admin/products?filter=low_stock" className="underline hover:text-orange-900">
              {t.admin.dashViewProducts}
            </Link>
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900">{t.admin.dashRecentOrders}</h2>
            <Link href="/admin/orders" className="text-xs text-blue-600 hover:underline flex items-center gap-1">
              {t.admin.dashViewAll} <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
          <div className="rounded-xl border border-gray-200 overflow-x-auto">
            {recentOrders.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-10">{t.admin.dashNoOrders}</p>
            ) : (
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900">{t.admin.dashUpcomingBookings}</h2>
          ...
          <div className="rounded-xl border border-gray-200 overflow-x-auto">
            {upcomingBookings.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-10">{t.admin.dashNoBookings}</p>
            ) : (
                  ...
                  {upcomingBookings.map((b) => (
                    <tr key={b.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-700 whitespace-nowrap">
                        {new Date(b.scheduledAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                        {" "}
