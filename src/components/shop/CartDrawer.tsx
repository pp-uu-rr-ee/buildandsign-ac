...
export function CartDrawer() {
  ...
  return (
    <Sheet>
      <SheetTrigger className="relative p-2 rounded-md text-gray-600 hover:text-blue-600 hover:bg-gray-100 transition-colors" aria-label="Shopping cart">
      ...
      <SheetContent side="right" className="w-full sm:w-96 flex flex-col p-0">
            ...
            <ul className="space-y-4">
              {items.map((item) => (
                <li key={item.productId} className="flex gap-3">
                  {/* Thumbnail */}
                  <div className="relative h-16 w-16 rounded-lg border border-gray-200 bg-gray-50 overflow-hidden shrink-0">
                    {item.imageUrl ? (
                      <Image
                        src={item.imageUrl}
                        alt={item.name}
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <div className="flex items-center border border-gray-200 rounded overflow-hidden">
                        <button
                          ...
                          className="px-2 py-0.5 text-gray-500 hover:bg-gray-100 text-sm transition-colors"
                        >
                          −
                        </button>
                        <span className="px-2 py-0.5 text-xs font-semibold border-x border-gray-200 min-w-[2rem] text-center">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() => updateQty(item.productId, item.quantity + 1)}
        ...
        {items.length > 0 && (
          <div className="border-t border-gray-200 px-5 py-4 space-y-3">
            ...
            <Link
              href="/cart"
              className="flex items-center justify-center w-full py-2 text-sm text-gray-500 hover:text-gray-700 transition-colors"
            >
              View full cart
            </Link>
          </div>
